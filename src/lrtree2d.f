c
c     lrtree2d - collection of subroutines for working with
c     level-restricted quadtrees
c
c
c     UPDATES
c     2022-06-01: switch to new child ordering
c                      3 | 4
c                      -----
c                      1 | 2
c      
...
c-----Global variables
      ...
      integer  nboxes, nlev
      integer  ichildbox(4,*), iparentbox(*)
      integer  nblevel(0:*), istartlev(0:*), iboxlev(*)
c-----Local variables
...
C
      do i = 0, nlev - 1
         istart = istartlev(nlev-i-1)
         ilength = nblevel(nlev-i-1)
         ilev = nlev-i-1
         istartc = istartlev(nlev-i)
         idim = 2**ilev
         nside = 2*idim
         do j = 1, ilength
            icol = 1 + mod(j-1,idim )
            irow = 1 + (j-1)/idim
            mybox = istart+(j-1)
            ic1 = istartc + 2*(icol-1) + (2*(irow-1))*nside
            ic2 = ic1 + 1
            ic3 = ic1 + nside
            ic4 = ic3 + 1
...
C             we could have problems.
                 ...
                 elseif (nb .eq. 4)then
                   if(ipar .eq. ichild(1) .or. ipar .eq. ichild(3))then
                       ifixflag = 1
                   end if
                 elseif (nb .eq. 6)then
                   if(ipar .eq. ichild(2) .or. ipar .eq. ichild(4))then
                       ifixflag = 1
                   end if
                 elseif (nb .eq. 7)then 
...
c         = 4 - needed more than 50 levels
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c$    use omp_lib      
      implicit none
...
c-----Global variables
      ...
      integer nblevel(0:*), iboxlev(*), istartlev(0:*)
      integer itemparray(*)
...
C            | N  | N  | N  | N  | N  | N  |
C            |----|----|----|----|----|----|
C            | W  |N12*|N12 |N12 |N12*| E  |
C            |----|----|----|----|----|----|
C            | W  |W24 | 3  | 4  |E13 | E  |
C            |----|----|----|----|----|----|
C            | W  |W24 | 1  | 2  |E13 | E  |
C            |----|----|----|----|----|----|
C            | W  |S34*|S34 |S34 |S34*| E  |
C            |----|----|----|----|----|----|
C            | S  | S  | S  | S  | S  | S  |
C            |----|----|----|----|----|----|
C
...
C     N12 marks boxes well separated from 3,4 but not 1,2
C     S34 marks boxes well separated from 1,2 but not 3,4
C     E13 marks boxes well separated from 1,4 but not 2,3
C     W24 marks boxes well separated from 2,3 but not 1,4
C
C     The boxes marked with an asterix are special cases. 
C     The lower left S34* box needs data from 2 -> W2 list.
C     The lower right S34* box needs data from 1 -> E1 list.
C     The upper left N12* box needs data from 4 -> W4 list.
C     The upper right N12* box needs data from 3 -> E3 list.
C
C     INPUT:
C
...
C
C***********************************************************************
      subroutine lrt2d_mklists2(ibox,inall,nnall,iynall,in12,nn12,iy12,
...
C     col,row of child 1 (lower left corner).
C
      icol = icolbox(ichildbox(1,ibox))
      irow = irowbox(ichildbox(1,ibox))
c
C     First do the free space case:
C     Set all of the counters to 0
...
C     of the box being checked.
               ...
               if(irowtest .eq. irow+3)then
                  nnall = nnall + 1
                  inall(nnall) = ichild
                  iynall(nnall) = icoltest - icol
               elseif(irowtest .eq. irow-2)then
                  nsall = nsall + 1
                  isall(nsall) = ichild
                  iysall(nsall) = icoltest - icol
               elseif(icoltest .eq. icol-2)then
                  nwall = nwall + 1
                  iwall(nwall) = ichild
                  iywall(nwall) = irowtest - irow
               elseif(icoltest .eq. icol+3)then
                  neall = neall + 1
                  ieall(neall) = ichild
                  iyeall(neall) = irowtest - irow
               elseif(irowtest .eq. irow+2)then
                  nn12 = nn12 + 1
                  in12(nn12) = ichild
                  iy12(nn12) = icoltest - icol
                  if(icoltest .eq. icol-1)then
                     nww4 = nww4 + 1
                     iww4(nww4) = ichild
                     iwy4(nww4) = irowtest - irow
                  endif
                  if(icoltest .eq. icol+2)then
                     nee3 = nee3 + 1
                     iee3(nee3) = ichild
                     iey3(nee3) = irowtest - irow
                  endif
               elseif(irowtest .eq. irow-1)then
                  ns34 = ns34 + 1
                  is34(ns34) = ichild
                  iy34(ns34) = icoltest - icol
                  if(icoltest .eq. icol-1)then
                     nww2 = nww2 + 1
                     iww2(nww2) = ichild
                     iwy2(nww2) = irowtest - irow
                  endif
                  if(icoltest .eq. icol+2)then
                     nee1 = nee1 + 1
                     iee1(nee1) = ichild
                     iey1(nee1) = irowtest - irow
                  endif
               elseif(icoltest .eq. icol-1)then
                  nw24 = nw24 + 1
                  iw24(nw24) = ichild
                  iy24(nw24) = irowtest - irow
               elseif(icoltest .eq. icol+2)then
                  ne13 = ne13 + 1
                  ie13(ne13) = ichild
                  iy13(ne13) = irowtest - irow
               endif
            enddo
         elseif(ichildbox(1,iout) .le. 0)then
            if(i .eq. 1)then
               isbig34(1) = iout
               iwbig2(1) = iout
               iflagnesw(3,iout) = 1
...
C
C***********************************************************************
      subroutine lrt2d_mklists(ibox,inall,nnall,iynall,in12,nn12,iy12,
     1    isall,nsall,iysall,is34,ns34,iy34,ieall,neall,
     2    iyeall,ie13,ne13,iy13,iwall,nwall,iywall,
     3    iw24,nw24,iy24,iww2,iwy2,nww2,iww4,iwy4,nww4,
     4    iee1,iey1,nee1,iee3,iey3,nee3,inbig12,isbig34,iebig13,
...
C-----Global variables
      ...
      integer  iflagnesw(4,*)
c-----Local variables
      integer  j, iout, ic1, ic2, ic3, ic4
C
C     Initially, set all list entries and offsets to zero.
C
      do j = 1, 6
      ...
      iout = icolleagbox(1,ibox)
      if (iout .le. 0) goto 2000
      if (ichildbox(1,iout) .gt. 0) then
 ...
 2000 continue 
      ...
      iout = icolleagbox(2,ibox)
      if (iout .le. 0) goto 3000
      if (ichildbox(1,iout) .gt. 0) then
         ic1 = ichildbox(1,iout)
         ic2 = ichildbox(2,iout)
         ic3 = ichildbox(3,iout)
 ...
 3000 continue 
      ...
      iout = icolleagbox(3,ibox)
      if (iout .le. 0) goto 4000
      if (ichildbox(1,iout) .gt. 0) then
         ic1 = ichildbox(1,iout)
         ic2 = ichildbox(2,iout)
         ic3 = ichildbox(3,iout)
 ...
 4000 continue 
      ...
      iout = icolleagbox(4,ibox)
      if (iout .le. 0) goto 6000
      if (ichildbox(1,iout) .gt. 0) then
         ic1 = ichildbox(1,iout)
         ic2 = ichildbox(2,iout)
         ic3 = ichildbox(3,iout)
 ...
 6000 continue 
      ...
      iout = icolleagbox(6,ibox)
      if (iout .le. 0) goto 7000
      if (ichildbox(1,iout) .gt. 0) then
         ic1 = ichildbox(1,iout)
         ic2 = ichildbox(2,iout)
         ic3 = ichildbox(3,iout)
 ...
 7000 continue 
      ...
      iout = icolleagbox(7,ibox)
      if (iout .le. 0) goto 8000
      if (ichildbox(1,iout) .gt. 0) then
         ic1 = ichildbox(1,iout)
         ic2 = ichildbox(2,iout)
         ic3 = ichildbox(3,iout)
 ...
 8000 continue 
      ...
      iout = icolleagbox(8,ibox)
      if (iout .le. 0) goto 9000
      if (ichildbox(1,iout) .gt. 0) then
         ic1 = ichildbox(1,iout)
         ic2 = ichildbox(2,iout)
         ic3 = ichildbox(3,iout)
 ...
 9000 continue 
      ...
      iout = icolleagbox(9,ibox)
      if (iout .le. 0) goto 9999
      if (ichildbox(1,iout) .gt. 0) then
         ic1 = ichildbox(1,iout)
         ic2 = ichildbox(2,iout)
         ic3 = ichildbox(3,iout)
...
C             we could have problems.
                    ...
                    elseif (nb .eq. 4)then
                      if(ibox.eq.ichild(1) .or. ibox.eq.ichild(3))then
                          iflag(itemp) = 2
                      end if
                    elseif (nb .eq. 6)then
                      if(ibox.eq.ichild(2) .or. ibox.eq.ichild(4))then
                          iflag(itemp) = 2
                      end if
                    elseif (nb .eq. 7)then
...
c          Scan colleague's children.
               do 300 icntr = 1, 4
c     if child of colleague a leaf, no need to subdivide...
                  if (ichildbox(1,ichild(icntr)) .lt. 0)goto 300
c     for subdivided children of colleagues, divide if necessary
                  if(jcntr .eq. 1 .and. icntr .eq. 4)then
c                    Call subdiv
                     if(ichildbox(1,ibox) .lt. 0)then
...
c                    flag++ all children created
                     ...
                     itemp = ichildbox(4,ibox)
                     iflag(itemp) = 3
                  elseif(jcntr .eq. 3 .and. icntr .eq. 3)then
c                    Call subdiv
                     if(ichildbox(1,ibox) .lt. 0)then
                        call lrt2d_subdiv(ibox,iparentbox,ichildbox,
     1                  icolleagbox,nboxes,irowbox,icolbox,levelbox,
...
c                    flag++ all children created
     ...
     1             (icntr .eq. 2 .or. icntr .eq. 4))then
c                    Call subdiv
                     if(ichildbox(1,ibox) .lt. 0)then
                        call lrt2d_subdiv(ibox,iparentbox,ichildbox,
     1                  icolleagbox,nboxes,irowbox,icolbox,levelbox,
...
c                    flag++ all children created
     ...
     1             (icntr .eq. 1 .or. icntr .eq. 3))then
c                    Call subdiv
                     if(ichildbox(1,ibox) .lt. 0)then
                        call lrt2d_subdiv(ibox,iparentbox,ichildbox,
     1                  icolleagbox,nboxes,irowbox,icolbox,levelbox,
...
c                    flag++ all children created
                     ...
                     itemp = ichildbox(4,ibox)
                     iflag(itemp) = 3
                  elseif(jcntr .eq. 7 .and. icntr .eq. 2)then
c                    Call subdiv
                     if(ichildbox(1,ibox) .lt. 0)then
                        call lrt2d_subdiv(ibox,iparentbox,ichildbox,
     1                  icolleagbox,nboxes,irowbox,icolbox,levelbox,
...
c                    flag++ all children created
                     ...
                     itemp = ichildbox(4,ibox)
                     iflag(itemp) = 3
                  elseif(jcntr .eq. 9 .and. icntr .eq. 1)then
c                    Call subdiv
                     if(ichildbox(1,ibox) .lt. 0)then
                        call lrt2d_subdiv(ibox,iparentbox,ichildbox,
     1                  icolleagbox,nboxes,irowbox,icolbox,levelbox,
...
c
      ...
      ichildbox(3,iparbox) = nboxes + 3
      ichildbox(4,iparbox) = nboxes + 4
